 '''
  export = filename.get() + '.csv'
    tables[0].to_csv(export)
    str=filename.get() + '.xls is created in current directory '
    print(str)


    filesavepath = filedialog.asksaveasfile(  )
    filesavename=str( filesavepath.name )
    fsavelist= filesavename.split('/')
    fsname = str (fsavelist[:-1] )
    fsrealnamelist = fsname.split('.')
    filesave = fsrealnamelist[0]
    str1 = filename.get() + '.xls is created in current directory '
    print(str1)

    #toexport= filename.get() + pageentry.get()+ '.csv'
    #tables[0].to_csv(toexport )
'''
 ...
 # -*- coding: utf-8 -*-
"""
Created on Tue Nov 19 18:47:28 2019
...
def gettable() :
    ...
    tables=camelot.read_pdf(file , password=passname.get())
    export = filename.get() + '.csv'
    tables[0].to_csv(export)
    ...
    try :
        ...
        #os.system(r"excel.exe" + filesavepath.name)

# start program
window = Tk()
window.title("Aztec Tabify")
window.config(background='skyblue')
azteclabel =Label(window , text=" Aztec Table ")
azteclabel.grid(column=0, row=1 , columnspan=6 ,pady=10 , padx=10)
azteclabel.config(font=("Courier", 56))
window.geometry("570x500")
#background


#row 1 elements
lbl = Label(window, text="Get table from pdf ", fg="blue", bg="yellow" ,  width=50 , height=2 )
lbl.grid(column=0, row=2 , columnspan=4)
lbl.config(font=("Courier", 14))


filelbl=Label(window , text='Enter file name  ', bg='skyblue').grid(row=4 , column=0)
filename=Entry(window)
filename.grid(row=4, column=1 )
filename.focus_set()

passlbl=Label(window , text='Enter Password  ' , bg='skyblue').grid(row=5 , column=0)
passname=Entry(window)
passname.grid(row=5, column=1 )

pdfpageslbl=Label(window , text='Number of pages  ' , bg='skyblue').grid(row=6 , column=0)
pdfpageentry=Entry(window)
pdfpageentry.grid(row=6, column=1 )

#function for displaying the result in terminal .
def result () :
    if filename.get()==None or pageentry.get()=="" or pdfpageentry.get()=="" :
        messagebox.showerror('Error' , "Invalid Responce ")
        return
...
pagelbl=Label(window , text='Enter table number ' , bg='skyblue'  ).grid(row=7 , column=0)
pageentry=Entry(window)
pageentry.grid(row=7, column=1 )

submitbtn=Button(window , text='Submit' , font = ( "fixedsys" , 8), command=result , activebackground='Light Green' , activeforeground='black' , height=1 , width=20)
submitbtn.grid(row=8,  columnspan = 5)

tablebtn = Button(window , text="Get ExcelFile" , font= ("fixedsys", 8) , command=gettable , activebackground='Light green' , width=20 , height=1  , activeforeground='white')
tablebtn.grid(row=9, columnspan=5 )
...
exitbtn= Button( window , text="Close" , font = ( "fixedsys" , 8), command=destroy , activebackground='Red' , activeforeground='black' , height=1 , width=20)
exitbtn.grid(row=10 , columnspan = 5)

#row 5 element has successful message
